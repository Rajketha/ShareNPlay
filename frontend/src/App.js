...
function App() {
  ...
  const qrUrl =
    `${window.location.origin}${window.location.pathname}?code=${code}&view=receiver`;
  ...
  useEffect(() => {
    axios.get('/dare-categories')
      .then(res => {
        setDareCategories(res.data);
      })
      .catch(err => {
        console.error('Error fetching dare categories:', err);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    s.on('gameStart', ({ gameType, round, maxRounds, playerMap, gameData, dares }) => {
      ...
      setFileInfo(prev =>
        prev
          ? { ...prev, gameSuggestion: { game: mappedGame } }
          : prev
      );
    ...
    s.on('roundResult', ({ result, scores, round, playerMap }) => {
      ...
      if (s.id === playerMap.player1) setPlayerRole('player1');
      else if (s.id === playerMap.player2) setPlayerRole('player2');
    });
  ...
  const downloadFile = async () => {
    try {
      ...
      const blob = new Blob([res.data], { type: fileInfo?.mimetype || 'application/octet-stream' });
  ...
  const handleReceiverCodeChange = async (e) => {
    ...
    if (val.length === 6) {
      try {
        const res = await axios.get(`/fileinfo/${val}`);
        setReceiverFileInfo(res.data);
        setReceiverError('');
      } catch (error) {
        console.error('Error fetching file info:', error);
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ marginBottom: '1.5em' }}>
              ...
              <button
                ...
                onClick={async () => {
                  const res = await axios.get(`/random-dare/${selectedDareCategory}`);
            ...
            <div style={{
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    setIsDownloading(true);
                    const res = await axios.get(`/download/${receiverCode}`, {
                      responseType: 'blob'
                    });
                    const blob = new Blob([res.data], {
                      type: receiverFileInfo?.mimetype || 'application/octet-stream'
                    });
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = receiverFileInfo?.fileName || 'download';
                    a.click();
                    window.URL.revokeObjectURL(url);
                  } catch (err) {
                    console.error(err);
            ...
            <>
              <div style={{ marginBottom: '1.5em' }}>
                ...
                <button
                  ...
                  onClick={async () => {
                    const res = await axios.get(`/random-dare/${receiverDareCategory}`);
